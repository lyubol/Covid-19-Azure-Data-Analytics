...
try:
   base_df = spark.read.format('parquet').option('header', True).load('/mnt/adlslirkov/covid/base/Covid19_Deaths')
...
deaths_raw_df = deaths_raw_df.withColumn('Key', concat_ws('-', deaths_raw_df['CountryCode'], deaths_raw_df['Date'].cast('String'))) 
...
deaths_raw_df = deaths_raw_df.filter(deaths_raw_df.Key != '#country+code')

# COMMAND ----------

# DBTITLE 1,Reorder columns
deaths_raw_df = deaths_raw_df.select('Key', 'Province/State', 'Country/Region', 'Lat', 'Long', 'Date', 'Value', 'CountryCode', 'RegionCode', 'SubRegionCode', 'IntermediateRegionCode')
...
append_df = deaths_raw_df.exceptAll(base_df)
...
append_df.write.mode('append').format('parquet').save('/mnt/adlslirkov/covid/base/Covid19_Deaths')
